...
class _CastSampleState extends State<CastSample> {
  ...
  late final ChromeCastController _controller;
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      // floatingActionButton: FloatingActionButton(onPressed: () => setState(() => _state = AppState.connected)),
      appBar: AppBar(
  ...
  @override
  void dispose() {
    debugPrint('KANTI   dispose');
    super.dispose();
    resetTimer();
  }

  Widget _handleState() {
    debugPrint('KANTI   _handleState');
    switch (_state) {
      case AppState.idle:
        resetTimer();
  ...
  Widget _mediaControls() {
    debugPrint('KANTI   _mediaControls');
  ...
  void resetTimer() {
    debugPrint('KANTI   resetTimer');
    _timer?.cancel();
    _timer = null;
  }

  void startTimer() {
    debugPrint('KANTI   startTimer');
  ...
  Future<void> _onButtonCreated(ChromeCastController controller) async {
    debugPrint('KANTI   _onButtonCreated');
    _controller = controller;
    await _controller.addSessionListener();
  }

  Future<void> _onSessionStarted() async {
    debugPrint('KANTI   _onSessionStarted');
  ...
  Future<void> _onRequestCompleted() async {
    debugPrint('KANTI   _onRequestCompleted');
    final playing = await _controller.isPlaying();
    if (playing == null) return;
  ...
  Future<void> _onRequestFailed(String? error) async {
    debugPrint('KANTI   _onRequestFailed');
